...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if(box is not None):
        ...
        print("box detected")
        for i in range(0,4):
        	newbox.append(box[i][0])
        newbox = np.array(newbox, dtype = "float32")
        v1 = box[1] - box[0]
        ang = angle(v1[0],[1,0])
        ang = ang / math.pi * 180
        a = four_point_transform(frame,newbox)
        a = np.invert(a)
        
        for i in range(0,4):
          print(i)
          b = rotateImage(a,90*i)
          #text = pytesseract.image_to_string(b, config="-l eng --oem 1 --psm 10")
          minConf = 70
          data = pytesseract.image_to_data(b, config="-l eng --oem 1 --psm 10", output_type=pytesseract.Output.DICT)
